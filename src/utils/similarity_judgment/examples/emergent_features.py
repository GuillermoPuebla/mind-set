...
dataset_folder = "datasets_examples/emergent_features/"

"""
To make this example work, we need the Emergent Features dataset in the system. If not present, we need to create it. Since this is just an example, we create a small version of it: 
"""
from src.generate_datasets.low_mid_level_vision.emergent_features.generate_dataset import (
    generate_all as emergent_features_generation,
)

if not Path(dataset_folder).exists():
    emergent_features_generation(output_folder=dataset_folder, num_samples=20)
...
"""
--------- APPROACH NUMBER 1: WRITE AN ACTUAL TOML FILE, AND PASS IT TO THE FUNCTION --------- 
You can actually write your own config file for this particular task. Notice that you only need to specify the values that you wish to change from the default file.
...
with open(os.path.dirname(__file__) + "/emergent_features_config.toml", "r") as f:
    toml_config = toml.load(f)
compute_distance(**toml_config)

"""
--------- APPROACH NUMBER 2: DO EVERYTHING IN PYTHON. --------- 
...
#         saving_folders=dict(
#             results_folder=f"results/similarity_judgment/high_level_vision/CSE_CIE_dots/{p}/"
#         ),
#         transformation={
#             "values": {"translation": False, "scale": False, "rotation": False}
#         },
#     )
