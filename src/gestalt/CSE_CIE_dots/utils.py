...
class DrawShape:
    def __init__(
        ...
        borders=False,
        line_col=None,
        resize_up_resize_down=False,
        borders_width=None,
        min_dist_bw_points=0,
        min_dist_borders=0,
    ):
    ...
    def get_empty_single(self):
        ...
        x0 = np.random.randint(
            ...
            self.img_size[0] - r - self.borders_width - self.min_dist_borders,
        )
        y0 = np.random.randint(
            0 + r + self.min_dist_borders + self.borders_width,
            self.img_size[1] - r - self.borders_width - self.min_dist_borders,
        )
    ...
    def create_canvas(self, img_size=None, borders=None):
        ...
        if self.background == "random":
            img_size = Image.fromarray(
                np.random.randint(0, 255, (img_size[1], img_size[0], 3)).astype(
                    np.uint8
                ),
                mode="RGB",
            )
        ...
        if borders:
            ...
            draw.line(
                [
                    (img_size.size[0] - 1, 0),
                    (img_size.size[0] - 1, img_size.size[1] - 1),
                ],
                fill=self.line_args["fill"],
                width=self.borders_width,
            )
    ...
    def apply_antialiasing(self, im):
        ...
        im = im.resize(
            (im.size[0] // self.antialiasing, im.size[1] // self.antialiasing),
    ...
    def draw_parentheses(
        self, p, d=45, circle_pos="left", draw=None, **kwargs
    ):  # p if x, y
        x, y = p
        ...
        if circle_pos == "right":
            bb = [(x, -r_minor + y), (r_major * 2 + x, y + r_minor)]
            draw.arc(
                bb, 180 - d, 180 + d, width=self.line_args["width"], fill=self.fill
            )
        if circle_pos == "up":
            bb = [(-r_minor + x, -r_major * 2 + y), (r_minor + x, y)]
            draw.arc(bb, 90 - d, 90 + d, width=self.line_args["width"], fill=self.fill)
        if circle_pos == "down":
            bb = [(-r_minor + x, y), (r_minor + x, r_major * 2 + y)]
            draw.arc(
                bb, 270 - d, 270 + d, width=self.line_args["width"], fill=self.fill
            )
    ...
    def get_rnd_colon(self, type="h", dist=None):
        ...
        while repeat:
            ...
            if (
                0 + r * 2 < x11 < self.img_size[0] - r * 2
                and 0 + r * 2 < y11 < self.img_size[1] - r * 2
                and 0 + r * 2 < x12 < self.img_size[0] - r * 2
                and 0 + r * 2 < y12 < self.img_size[1] - r * 2
            ):
                repeat = False
            else:
    ...
    def get_char_canvas_with_rot(
        ...
        draw.text(
            (sz_fact[0] / 2, sz_fact[1] / 2),
            char,
            font=arial,
            anchor="mm",
            fill=(self.line_col,) * 3,
        )
    ...
    def get_beak(self, type):
        ...
        draw.arc(
            [
                (self.img_size[0] // 2 - 35, self.img_size[1] // 2 - 30),
                (self.img_size[0] // 2 + 10, self.img_size[1] // 2 + 30),
            ],
            0,
            360,
        )
    ...
    def get_triangle(self, type="base_0"):
        ...
        if type == "base_0":
            ...
            radius = np.round(
                np.sqrt(
                    (im.size[0] * 1 // 4) * (im.size[0] * 1 // 4)
                    + (im.size[0] * 1 // 4) * (im.size[0] * 1 // 4)
                )
            )
            l = self.center_at_cavas(
                self.from_radians_get_line(radius, t / 360 * (2 * np.pi))
            )
            draw = ImageDraw.Draw(im)
    ...
    def get_proximity_points(self, pp=None, dist=None, **kwargs):
        ...
        while not stop:
            ...
            if (
                ...
                and self.img_size[0] - r - self.borders_width - self.min_dist_borders
                > xx_lin
                > 0 + r + self.borders_width + self.min_dist_borders
                < yy_lin
                < self.img_size[1] - r - self.borders_width - self.min_dist_borders
    ...
    def two_points_and_equidistance_point(self):
        ...
        while not stop:
            ...
            distance = np.linalg.norm(
                np.array([x_base_0, y_base_0]) - np.array([x_base_1, y_base_1])
            )
            radius_circles = np.random.uniform(distance / 2, diagonal * 0.7)

            x1, y1, x2, y2 = self.intersections(
                x_base_0, y_base_0, radius_circles, x_base_1, y_base_1, radius_circles
            )
            ...
            if (
                0 + r < xx_equi < self.img_size[0] - r
                and 0 + r < yy_equi < self.img_size[1] - r
            ):
                stop = True
    ...
    def get_rnd_symmetry_p(self):
        ...
        (
            ...
            xx_equi,
            yy_equi,
        ) = self.two_points_and_equidistance_point()
        ...
        jitter = lambda: np.random.randint(
            -self.img_size[0] / 10, self.img_size[0] / 10
        )
        ...
        self.circle(
            draw,
            (
                np.clip(symm_point_ax + jitter(), 0 + r, self.img_size[0] - r),
                np.clip(y_base_0 + jitter(), 0 + r, self.img_size[0] - r),
            ),
            radius=r,
        )
        self.circle(
            draw,
            (
                np.clip(symm_point_bx + jitter(), 0 + r, self.img_size[0] - r),
                np.clip(yy_equi + jitter(), 0 + r, self.img_size[0] - r),
            ),
            radius=r,
        )
        # im_comp_asymm.show()
    ...
    def get_parentheses_crossed_set(self, type, offset=0):
        ...
        if type == "composite_0":
            p = [
                self.img_size[0] / 2
                + self.img_size[0] * offset
                - self.img_size[0] * 0.09,
                self.img_size[1] / 2,
            ]
            self.draw_parentheses(p, 35, "right", draw=draw)
            p = [
                self.img_size[0] / 2 - self.img_size[0] * offset,
                self.img_size[1] / 2 + 20,
            ]
    ...
    def get_array1(self):
        ...
        draw = ImageDraw.Draw(im1)
        s = 7
        self.draw_line_with_circled_edge(
            draw,
            (
                (
    ...
    def get_array6(self):
        ...
        self.draw_line_with_circled_edge(
            ...
            (
                (
                    self.img_size[0] // 2 - self.img_size[1] // 2 // s,
                    self.img_size[1] // 2,
                ),
                (
                    self.img_size[0] // 2 + self.img_size[1] // 2 // s,
    ...
    def get_array12(self):
        ...
        self.draw_line_with_circled_edge(
            ...
            (
                ...
                (
                    self.img_size[0] // 2,
                    self.img_size[1] // 2 - (self.img_size[0] // s3 // s),
                ),
            ),
        )
        ...
        self.draw_line_with_circled_edge(
            ...
            (
                ...
                (
                    self.img_size[1] // 2 - (self.img_size[0] // s2 // s),
                    self.img_size[1] // 2 - (self.img_size[0] // s3 // s),
                ),
            ),
        )
    ...
    def get_array14(self):
        ...
        self.draw_line_with_circled_edge(
            ...
            (
                (
                    self.img_size[0] // 2,
                    self.img_size[1] // 2 - (self.img_size[1] // 2 // (s * 2)),
                ),
                (
                    self.img_size[0] // 2,
                    self.img_size[1] // 2 + (self.img_size[1] // 2 // (s * 2)),
    ...
    def get_arrayE(self, dist_factor=10):
        ...
        p = [
            self.img_size[0] / 2
            + 1.35 * self.img_size[0] / dist_factor
            - self.img_size[0] * 0.067,
            self.img_size[1] / 2,
        ]
        self.draw_parentheses(p, 35, "right", draw=draw)
    ...
    def get_arrayE2(self, dist_factor=10):
        ...
        def create_context():
            p = [
                self.img_size[0] / 2 - self.img_size[0] / 10 - self.img_size[0] * 0.2,
                self.img_size[1] / 2,
            ]
            self.draw_parentheses(p, 35, "right", draw=draw)

            p = [
                self.img_size[0] / 2 - self.img_size[0] / 10 + dist_factor,
                self.img_size[1] / 2,
            ]
            self.draw_parentheses(p, 35, "left", draw=draw)
    ...
    def get_square_rotarray2(self, rotate_segm=30):
        ...
        self.draw_line_with_circled_edge(
            draw, (tuple(xy0), tuple(rot_vect)), width=self.line_args["width"]
        )
    ...
    def get_array2_rotarray2_allseg(self, rotate_segm=30):
        ...
        self.draw_line_with_circled_edge(
            draw, (tuple(xy1), tuple(rot_vect)), width=self.line_args["width"]
        )
        xy2 = np.array(
            [
                self.img_size[0] // 2 + (self.img_size[0] // 2 // s),
    ...
    def get_curly_composite_with_space(self, space=0):
        ...
        def add_curly_bracket(self, im):
            ...
            im.paste(
                cc,
                (
                    self.img_size[0] // 2 - sz_fact[0] // 2 - 30,
    ...
    def get_brackets_base(self):
        sz = (450, 450)
        im1 = self.get_char_canvas_with_rot(
            "(", sz_fact=sz, font_size=240, font_path="./AmasisMTpro.ttf"
        )
        im2 = self.get_char_canvas_with_rot(
            ")", sz_fact=sz, font_size=240, font_path="./AmasisMTpro.ttf"
        )
        return im1, im2
    ...
    def get_array11_curly(self):
        ...
        im1.paste(
            cc,
            (
                self.img_size[0] // 2 - sz_fact[0] // 2 - 10,
