...
def generate_all(output_folder, canvas_size, background, antialiasing):
    ...
    output_folder = (
        pathlib.Path("data") / "gestalt" / "leuven_embedded_figures_test"
    ...
    with open(output_folder / "annotation.csv", "w", newline="") as annfile:
        ...
        for idx, s in enumerate(all_shapes_path):
            img = load_and_invert(s, canvas_size, background, antialiasing)
            folder = output_folder_shape / str(idx)
            n = get_highest_number(folder)
            img.save(folder / f"{n + 1}.png")
            writer.writerow(
                [f"shapes/{str(idx)}/{n + 1}.png", "shapes", idx, background]
            )

            ## Test figures
            # Here we only take the figures containing the "target" shape, not the figures with the distractor (which are also provided in the dataset). That's because the standard test consits of checking whether a network can correctly classify these stimuli with a high accuracy.
