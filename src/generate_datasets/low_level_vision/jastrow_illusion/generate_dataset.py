...
class DrawJastrow(DrawStimuli):
    def generate_jastrow_illusion(self, arc, width, size_red, size_blue, top_color, type_stimulus):
        ...
        while not correct_stimulus:
            ...
            arc_1.move_to(position_fun()).rotate(rotation_fun()) if type_stimulus == "random" else None

            arc_2 = Shapes(parent=parent)
            arc_2.add_arc(size=arcs_sizes[1], arc=arc, width=width)
            arc_2.move_to(position_fun()).rotate(rotation_fun()) if type_stimulus == "random" else None
            ...
            if type_stimulus == "aligned" or type_stimulus == "illusory":
                ...
                while not image_ok:
                    ...
                    if not (arc_1.is_touching(arc_2) and parent.compute_jastrow_factor().round(3) > 0.7):
                        image_ok = True
...
def generate_all(
    ...
    [
        [
            (output_folder / type / ("" if type == "random" else f"{top}_on_top")).mkdir(exist_ok=True, parents=True)
            for type in types
        ]
