...
class DrawLinedrawings(DrawStimuli):
    ...
    def get_linedrawings(self, image_path, type):
        ...
        if len(str(image_path).split("bear")) == 3:
            stop = 1
        ...
        new_canvas = Image.fromarray(
            np.ones((img.shape[0], img.shape[1] * 2)) * 255
        ).convert("RGB")

        new_canvas.paste(top_half, (offset, 0))
        new_canvas.paste(bottom_half, (0, height // 2))

        source_canvas = new_canvas
        canvas = self.create_canvas()
        mask = ImageOps.invert(source_canvas.convert("L"))

        new_image = Image.new("RGBA", source_canvas.size, self.fill)

        result = ImageChops.composite(new_image, canvas, mask)

        if type == "fragmented":
            center_point = [bottom, img.shape[0] // 2]
        else:
            center_point = np.array(Image.fromarray(img).size) // 2

        leftmost_x = canvas.size[0] // 2 - center_point[0]
        leftmost_y = canvas.size[1] // 2 - center_point[1]
        position = (leftmost_x, leftmost_y)

        result = result.convert("RGBA")

        canvas.paste(result, position, result)
