...
class DrawShape:
    ...
    def create_canvas(self, img=None, borders=None):
        ...
        if self.background == "random":
            img = Image.fromarray(
                np.random.randint(0, 255, (img[1], img[0], 3)).astype(np.uint8),
                mode="RGB",
            )
    ...
    def create_random_ebbinghaus(
        ...
        for i in range(n):
            random_points = [
                np.random.randint(self.img_size[0]),
                np.random.randint(self.img_size[1]),
            ]
            random_size = self.img_size[0] * gen_rnd(flankers_size_range)
            self.circle(draw, np.array(random_points), random_size)
        self.circle(
            draw, self.img_size / 2, self.img_size[0] * r_c, fill=colour_center_circle
        )
