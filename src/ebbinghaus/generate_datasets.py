...
class EbbinghausRandomFlankers(StaticDataEbbinghaus):
    def __init__(self, img_size, *args, background="black", **kwargs):
        ...
        ds = DrawShape(
            background=background, img_size=img_size, resize_up_resize_down=True
        )
...
EbbinghausRandomFlankers(
    path=f"./{args.folder}/random_data_n{args.num_training_data}/train",
    size_dataset=args.num_training_data,
    img_size=224,
    background="black",
)

EbbinghausRandomFlankers(
    path=f"./{args.folder}/random_data_n{args.num_testing_data}/test",
    size_dataset=args.num_testing_data,
    img_size=224,
    background="black",
)

EbbinghausTestSmallFlankers(
    path=f"./{args.folder}/small_flankers_data_n{args.num_testing_data}/test",
    size_dataset=args.num_testing_data,
    img_size=224,
    background="black",
)
...
EbbinghausTestBigFlankers(
    path=f"./{args.folder}/big_flankers_data_n{args.num_testing_data}/test",
    size_dataset=args.num_testing_data,
    img_size=224,
    background="black",
)
