# """
# This tests for the ebbinghaus_illusion illusion. Run it after having run "train".
...
#
# import torch.nn
# from src.ebbinghaus_illusion.generate_datasets import (
#     EbbinghausRandomFlankers,
#     EbbinghausTestBigFlankers,
#     EbbinghausTestSmallFlankers,
# )
...
# parser.add_argument(
#     "--gpu_num", help="what gpu to use (int, starts at 0)?", type=int, default=0
# )
...
#
# test_dataset = fix_dataset(
#     RegressionDataset("./data/ebbinghaus_illusion/test_random_data"), "test"
# )
# test_dataset_small = fix_dataset(
#     RegressionDataset("./data/ebbinghaus_illusion/test_small_flankers_data"), "test_small"
# )
# test_dataset_big = fix_dataset(
#     RegressionDataset("./data/ebbinghaus_illusion/test_big_flankers_data"), "test_big"
# )
#
# num_decoders = len(config.net.decoders)
#
...
#         loss = make_cuda(torch.tensor([0.0]), use_cuda)
#         mse_dec = []
#         for idx, od in enumerate(out_dec):
#             mse_dec.append(loss_fn(od, labels))
#             loss += mse_dec[-1]
...
#         [
#             logs[f"me_list_{i}"].extend((out_dec[i] - labels).tolist())
#             for i in range(len(out_dec))
#         ]
#
...
#         sty.fg.red
#         + sty.ef.inverse
#         + f"***** DATASET {loader.dataset.name_ds} *****"
#         + sty.rs.fg
#         + sty.rs.ef
#     )
#     _, logs = run(
...
#         StopFromUserInput(),
#         ProgressBar(
#             l=len(test_dataset_big),
#             batch_size=config.batch_size,
#             logs_keys=[f"ema_rmse_{i}" for i in range(num_decoders)],
#         ),
#         StopWhenMetricIs(
#             value_to_reach=0, metric_name="epoch", check_after_batch=False
#         ),
#     ]
#
#
# out_decoders = {
#     tl.dataset.name_ds: call_run(tl, train=False, callbacks=gen_cb())
...
#         )
#         plt.axvline(
#             np.median(out_decoders[n][decnum]), linestyle="--", color=color_cycle[idx]
#         )
#
#     plt.legend()
